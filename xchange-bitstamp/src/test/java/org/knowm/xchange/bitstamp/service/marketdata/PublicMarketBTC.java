...
public class PublicMarketBTC {

  Logger logger = LoggerFactory.getLogger(PublicMarketBTC.class);
  ...
  @Test
  public void testArbitrageETH_BTC_JPY() {
    PublicMarketBTC pm = new PublicMarketBTC();
    while (true) {

      try {
        Thread.sleep(5000l);
        pm.arbitrage(CurrencyPair.ETH_BTC, CurrencyPair.ETH_JPY, CurrencyPair.BTC_JPY);
      } catch (Exception e) {
        logger.warn("error", e);
      }
    }
