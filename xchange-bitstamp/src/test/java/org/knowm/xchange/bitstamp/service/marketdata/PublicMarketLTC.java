...
public class PublicMarketLTC {

  Logger logger = LoggerFactory.getLogger(PublicMarketLTC.class);
  ...
  @Test
  public void testArbitrageLTC_BTC_EUR() {
    PublicMarketLTC pm = new PublicMarketLTC();
    while (true) {

      try {
        Thread.sleep(5000l);
        pm.arbitrage(CurrencyPair.LTC_BTC, CurrencyPair.LTC_EUR, CurrencyPair.BTC_EUR);
      } catch (Exception e) {
        logger.warn("error", e);
      }
    }
