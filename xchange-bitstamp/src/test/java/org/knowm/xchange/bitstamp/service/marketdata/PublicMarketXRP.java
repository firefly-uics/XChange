...
public class PublicMarketXRP {

  Logger logger = LoggerFactory.getLogger(PublicMarketXRP.class);
  ...
  @Test
  public void testArbitrageXRP_BTC_EUR() {
    PublicMarketXRP pm = new PublicMarketXRP();
    while (true) {

      try {
        Thread.sleep(5000l);
        pm.arbitrage(CurrencyPair.XRP_BTC, CurrencyPair.XRP_EUR, CurrencyPair.BTC_EUR);
      } catch (Exception e) {
        logger.warn("error", e);
      }
    }
