...
public class PublicMarketDOGE {

  Logger logger = LoggerFactory.getLogger(PublicMarketDOGE.class);
  ...
  public void testArbitrage() {
    PublicMarketDOGE pm = new PublicMarketDOGE();
    while (true) {

      try {
        Thread.sleep(5000l);
        pm.arbitrage(CurrencyPair.DOGE_BTC, CurrencyPair.DOGE_USD, CurrencyPair.BTC_USD);
      } catch (Exception e) {
        logger.warn("error", e);
      }
    }
