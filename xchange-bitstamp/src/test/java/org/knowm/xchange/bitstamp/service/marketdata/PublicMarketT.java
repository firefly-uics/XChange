...
public class PublicMarketT {

  Logger logger = LoggerFactory.getLogger(PublicMarketT.class);
  ...
  public void testArbitrage() {
    PublicMarketT pm = new PublicMarketT();
    while (true) {

      try {
        Thread.sleep(1000l);
        pm.arbitrage(CurrencyPair.ETH_BTC, CurrencyPair.ETH_USD, CurrencyPair.BTC_USD);
      } catch (Exception e) {
        logger.warn("error", e);
      }
    }
